...
export default function UserProfile() {
  ...
  useEffect(() => {
    const fetchUserProfile = async () => {
      try {
        ...
        setSelectedModel(response.data.model || 'default');
  ...
  const stats = useMemo(() => {
    ...
    return [...user.dailyStats].sort((a, b) => 
  ...
  const yearlyData = useMemo(() => getYearlyData(stats, yearFilter), [stats, yearFilter]);
  ...
  const handleModelChange = async (e) => {
    ...
    try {
      ...
      setUser(prev => ({ ...prev, model }));
    } catch (error) {
      alert('Napaka pri posodabljanju modela');
      setSelectedModel(user?.model || 'default');
    } finally {
    ...
    <div style={{ width: '100%',maxWidth: '1400px', margin: '0 auto', padding: '20px'}}>
      ...
      <div style={{ 
        ...
        <div style={{ marginTop: '20px' }}>
          ...
          {updatingModel && <span style={{ marginLeft: '10px' }}>Shranjujem...</span>}
        </div>
      </div>
        ...
        {latestStat ? (
          <>
            ...
            <div style={{ display: 'flex', justifyContent: 'space-around', marginTop: '20px' }}>
              <div><strong>Razdalja:</strong><br />{distance} km</div>
              <div><strong>Kalorije:</strong><br />{calories}</div>
              <div><strong>Hitrost:</strong><br />{latestStat.speed ? `${latestStat.speed.toFixed(2)} km/h` : 'Ni podatka'}</div>
              <div><strong>Višina:</strong><br />{latestStat.altitude ? `${latestStat.altitude.toFixed(0)} m` : 'Ni podatka'}</div>
              <div><strong>Datum:</strong><br />{new Date(latestStat.date).toLocaleDateString('sl-SI')}</div>
            </div>

            <div style={{ marginTop: '30px' }}>
              <label htmlFor="stepGoalInput"><strong>Nastavi dnevni cilj korakov:</strong></label>
      ...
      <div style={{ 
        ...
        <div style={{ display: 'flex', justifyContent: 'space-around', marginTop: '20px' }}>
          <div><strong>Skupaj korakov:</strong><br />{lifetimeStats.totalSteps.toLocaleString('sl-SI')}</div>
          <div><strong>Skupaj prehojeno:</strong><br />{lifetimeStats.totalDistance.toFixed(2)} km</div>
          <div><strong>Porabljenih kalorij:</strong><br />{lifetimeStats.totalCalories.toFixed(0)}</div>
          <div><strong>Dnevov s podatki:</strong><br />{lifetimeStats.daysCount}</div>
        </div>
      </div>
